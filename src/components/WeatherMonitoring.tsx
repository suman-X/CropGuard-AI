...
export const WeatherMonitoring = () => {
  ...
  const getRiskIcon = (riskLevel: string) => {
    switch (riskLevel) {
      ...
      default:
        return <CheckCircle2 className="h-5 w-5 text-zinc-100" />;
    }
  };

  if (!weatherAlert) {
    return (
      <Card className="mb-8 border border-zinc-800 bg-zinc-950">
  ...
  return (
    <Card className={`mb-8 border border-zinc-800 bg-zinc-950 ${weatherAlert.riskLevel === 'Critical' ? 'border-red-500/50 bg-red-500/5' :
      weatherAlert.riskLevel === 'High' ? 'border-red-500/30 bg-red-500/5' :
        weatherAlert.riskLevel === 'Medium' ? 'border-yellow-500/50 bg-yellow-500/5' :
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className="bg-zinc-900 border-zinc-700 text-zinc-100 hover:bg-zinc-800 hover:text-white transition-colors"
          >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex items-center gap-2 p-3 bg-zinc-900 rounded-lg border border-zinc-800">
            <Thermometer className="h-4 w-4 text-zinc-100" />
            <div>
              <div className="text-xs text-zinc-400">Temperature</div>
              <div className="font-semibold text-zinc-100">{weatherAlert.temperature}°C</div>
            </div>
          </div>
          <div className="flex items-center gap-2 p-3 bg-zinc-900 rounded-lg border border-zinc-800">
            <Droplets className="h-4 w-4 text-zinc-100" />
            <div>
              <div className="text-xs text-zinc-400">Humidity</div>
              <div className="font-semibold text-zinc-100">{weatherAlert.humidity}%</div>
            </div>
          </div>
          <div className="flex items-center gap-2 p-3 bg-zinc-900 rounded-lg border border-zinc-800">
            <Cloud className="h-4 w-4 text-zinc-100" />
            <div>
              <div className="text-xs text-zinc-400">Condition</div>
              <div className="font-semibold text-zinc-100">{weatherAlert.condition}</div>
            </div>
          </div>
          {weatherAlert.windSpeed && (
            <div className="flex items-center gap-2 p-3 bg-zinc-900 rounded-lg border border-zinc-800">
              <Wind className="h-4 w-4 text-zinc-100" />
              <div>
                <div className="text-xs text-zinc-400">Wind Speed</div>
                <div className="font-semibold text-zinc-100">{weatherAlert.windSpeed} m/s</div>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Risk Level */}
        <Alert className="border-zinc-800 bg-zinc-900/50 text-zinc-100">
          <AlertDescription className="flex items-center justify-between">
            <span className="font-semibold text-zinc-100">Risk Level:</span>
            <Badge variant={getSeverityColor(weatherAlert.riskLevel)} className="text-sm font-bold">
        ...
        {weatherAlert.alerts.length > 0 && (
          <div>
            <h4 className="font-semibold mb-2 flex items-center gap-2 text-zinc-100">
              <AlertTriangle className="h-4 w-4 text-amber-500" />
              Current Alerts
            </h4>
            <ul className="space-y-2 text-sm">
              {weatherAlert.alerts.map((alert, index) => (
                <li key={index} className="flex items-start gap-2 p-2 rounded bg-zinc-900/50 border border-zinc-800/50">
        ...
        {weatherAlert.pestTypes.length > 0 && (
          <div>
            <h4 className="font-semibold mb-2 text-sm text-zinc-100">Pests to Watch For:</h4>
            <div className="flex flex-wrap gap-2">
              {weatherAlert.pestTypes.map((pest, index) => (
                <Badge key={index} variant="outline" className="text-xs border-zinc-700 text-zinc-300 bg-zinc-900/50">
                  {pest}
                </Badge>
              ))}
            </div>
